        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
              ...
              var model = self.options.camelCase ? _.camelCase(self.tables[table][field][attr].target_table) : self.tables[table][field][attr].target_table;
              var key = self.options.camelCase ? _.camelCase(self.tables[table][field][attr].target_column) : self.tables[table][field][attr].target_column;
              text[table] += spaces + spaces + spaces + "references: {\n";
              text[table] += spaces + spaces + spaces + spaces + "model: \'" + model + "\',\n"
              text[table] += spaces + spaces + spaces + spaces + "key: \'" + key + "\'\n"
